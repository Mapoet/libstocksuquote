...
libstocks_return_code http_get(char *http_file, char *http_server, char **pdata)
{
  ...
  if (http_proxy_server)
    {
        sprintf(header,"GET http://%.128s:80%.256s HTTP/1.0\015\012host: %s\015\012\015\012", http_server, http_file, http_server);
    }
  else
    {
        sprintf(header,"GET %s HTTP/1.0\015\012host: %s\015\012\015\012", http_file, http_server);
    }
